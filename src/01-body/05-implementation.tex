...
We first give an explanation to some major decisions taken before designing and implementing \OurBenchmarkingTool~in \Cref{sec:impl.decisions}.
We describe the system design in detail in \Cref{sec:impl.design}.
We then conclude the chapter with the detail of our implementation of \OurBenchmarkingTool~in \Cref{sec:impl.impl}.
...
We can see from \Cref{ch:existing} that Python is chosen by four out of the six evaluated benchmarking tool as their implementation language.
...
These packages can be installed through \code{pip} tool, which is often distributed alongside Python.
...
We decide to use \O MQ because it differs from other messaging queue solutions in the way that it is a library instead of separate tools.
...
\newcontent{
    Unlike typical client-server architecture where the server do most of the computation, the client-server pattern in our design is used as a separation between computation and data collection.
    The workload is distributed to the \emph{workers} in the client-side which report the results to the server to collect.
}
The client manages the heavy computation while the server manages the data.
\newcontent{A recommended setup is for the client to be in a high performance computing (HPC) cluster system while the server in a separate system, for example a virtual private server (VPS).}
...
Assuming the server is already started, the figure illustrates the steps done after the user executes the \code{manage [manager] run} command.
...
The client then spawns workers with the same number as pending benchmark runs.
...
The benchmarking process is modeled as in the entity-relationship diagram \citep{chen1976entity} shown in Figure \ref{fig:erd}.
...
Publishing to pypi enable wide usage since the tools for downloading through pypi, \code{pip} is shipped alongside Python.
...
\textsc{greenlet} (\url{https://github.com/python-greenlet/greenlet}) is a Python package that provides the use of greenlets.
...
First it calls the \mintinline{python}{def prepare(self)} method which setups SSH tunneling if it is used.
Then it runs the \mintinline{python}{def bootstrap(self)}, reading the configuration file, preparing tools and task sources, and then send a \code{BOOTSTRAP} event to the server.
...
The \textsc{slurm} manager works in the \textsc{slurm} cluster system \citep{yoo2003slurm} and also acts as a reference implementation for other potential managers.
...
We provide a simple utility to monitor the progress of the benchmark, \code{reprobench status}.
...
Typically after the benchmark completes and all the runs executed, the resulting database can be queried for analysis.
This can also be done live in the middle of benchmarking, but we do not provide any implementation for it yet.
The \code{reprobench analyze} command will execute the specified analysis steps with the database.
...
These descriptions can also be accessed behind the \code{--help} option.
