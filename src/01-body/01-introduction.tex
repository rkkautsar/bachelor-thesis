...
\newcontent{
	...
	This example shows that experiment can also take place in the mind and do not have to be physical.
}

Experiments has also demonstrated that artificial neural network performance is better than \newstyle{what was predicted in theory} \citep{tichyShouldComputerScientists1998}.
...
And last but not least, Isaac Eddington has done an expensive validation in 1919 to prove Einstein's theory that gravity bends light.
...
In this chapter, we list the research questions for this thesis foremost in \Cref{sec:intro.questions}.
Then we show how we answer these questions in \Cref{sec:intro.method}.
Our contribution to this topic is then listed in \Cref{sec:intro.contributions}.
We also mention some study related to this thesis in \Cref{sec:intro.related}.
Finally, we outline the content for the rest of this thesis in \Cref{sec:intro.outline}.
...
\begin{enumerate}
	...
	\item In \Cref{ch:resource}, we discusses the process of resource monitoring, the difficulties, how it relate to the benchmarking process and survey some existing implementations.
