...
\newcontent{
    ...
    \citet{beyerBenchmarkingResourceMeasurement2015} mention three limitations of commonly used methods compared to their CGroups-based implementation.
    ...
    \begin{enumerate}
        \item \textbf{Measuring resources may fail.}
        ...
        For example, \code{getrusage} only report the resource usage of child processes \code{wait}-ed by their parents \citep[getrusage]{manpages}

        \item \textbf{Enforcing limits may fail.}
        \lipsum[1-1]

        \item \textbf{Killing processes may fail.}
        \lipsum[1-1]
...
\newcontent {
    ...
    Note that all of the existing implementation \first~selected is developed for Linux and may not work for other UNIX-like OSes.
...
\newcontent{
    \textsc{runlim}\footlink{http://fmv.jku.at/runlim/} uses recursive sampling to monitor the resource usage.
    ...
    It uses \code{procfs} for all resource queries including querying the process list.
    The source code is published and licensed under the 4-clause BSD License.
    Not that it has not been updated since 2011 and the source code itself is not managed in a public version control system.
